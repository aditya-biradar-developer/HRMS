...
class EmailService {
  constructor() {
    ...
    if (!this.isConfigured) {
      console.warn('⚠️ Email service not configured. Set SENDGRID_API_KEY or EMAIL_USER/EMAIL_PASSWORD in .env file');
      this.transporter = null;
      return;
    }

    // Configure email transporter for SMTP fallback
    try {
    ...
    try {
      if (this.useResend) {
        ...
        const response = await axios.post('https://api.sendgrid.com/v3/mail/send', {
          ...
          from: {
            email: this.emailUser || 'noreply@ai-hrms.com',
            name: 'AI-HRMS'
          },
          content: [{
            type: 'text/html',
