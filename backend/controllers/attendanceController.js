...
const getAllAttendance = async (req, res) => {
  try {
    ...
    // Get all users to filter out admin and candidate attendance
    const User = require('../models/User');
    const allUsers = await User.getAll();
    
    // Filter out admin and candidate attendance records
    ...
    console.log('🔍 Filtering attendance:', {
      ...
      excludedRoles: ['admin', 'candidate']
    });
    
    // Filter attendance to exclude admin and candidate records
    attendance = attendance.filter(record => 
      employeeUserIds.includes(record.user_id)
    );
    
    console.log('✅ After role filtering:', attendance.length);
    ...
    if (userRole === 'manager') {
      ...
      console.log('👔 Manager filtering attendance:', {
        managerDept,
        recordsAfterRoleFilter: attendance.length
      });
      
      // Get users in manager's department (already filtered to exclude admin/candidate)
      ...
      console.log('✅ After department filtering:', attendance.length);
    }
