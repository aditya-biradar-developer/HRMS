...
const UsersPage = () => { // Rename component to avoid conflict
  ...
  // Auto-set department to 'Candidate' when role is candidate
  useEffect(() => {
    if (formData.role === ROLES.CANDIDATE) {
      setFormData(prev => ({ ...prev, department: 'Candidate' }));
    } else if (formData.department === 'Candidate' && formData.role !== ROLES.CANDIDATE) {
      // Reset to first department if role changed from candidate to something else
              ...
              <form onSubmit={handleSubmitUser} className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    className={`flex h-10 w-full rounded-md border border-input px-3 py-2 text-sm ring-offset-background ${
                      formData.role === ROLES.CANDIDATE 
                        ? 'bg-gray-100 cursor-not-allowed' 
                        : 'bg-background'
                    }`}
                    disabled={formData.role === ROLES.CANDIDATE}
                    required
                  >
                  ...
                  {formData.role === ROLES.CANDIDATE && (
                    <p className="text-xs text-gray-500">
                      Candidates are automatically assigned to the 'Candidate' department. This will be updated when they are hired.
                    </p>
                  )}
                </div>
                {formData.role !== ROLES.CANDIDATE && (
