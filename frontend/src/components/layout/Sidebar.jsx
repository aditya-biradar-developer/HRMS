...
const Sidebar = () => {
  ...
  const [pinnedNotification, setPinnedNotification] = useState(null);
            ...
            {filteredMenuItems.map((section) => (
              <div key={section.title} className="mb-4">
                ...
                {section.items.map((item) => {
                  ...
                  const notificationType = item.path.replace('/', ''); // Convert '/applications' to 'applications'
                  return (
                      ...
                      >
                        ...
                        {notificationCount > 0 && (
                          <div className="relative">
                            <span 
                              className="inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-white bg-red-500 rounded-full min-w-[20px] cursor-pointer hover:bg-red-600 transition-colors"
                              onMouseEnter={() => {
                                if (pinnedNotification !== notificationType) {
                                  setHoveredNotification(notificationType);
                                }
                              }}
                              onMouseLeave={() => {
                                if (pinnedNotification !== notificationType) {
                              ...
                              onClick={(e) => {
                                ...
                                if (pinnedNotification === notificationType) {
                                  ...
                                  setPinnedNotification(notificationType);
                                  setHoveredNotification(null);
                                }
                              }}
                              title={pinnedNotification === notificationType ? "Click to unpin" : "Click to pin notification"}
                            >
                              {notificationCount > 99 ? '99+' : notificationCount}
                              {pinnedNotification === notificationType && (
                            ...
                            <NotificationTooltip
                              ...
                              isVisible={hoveredNotification === notificationType || pinnedNotification === notificationType}
                              isPinned={pinnedNotification === notificationType}
                              onClose={() => {
                                setPinnedNotification(null);
                                setHoveredNotification(null);
                              }}
